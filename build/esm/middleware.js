                        ...
                        switch (payload.type) {
                            case 'alias':
                                return [2 /*return*/, this.wrapper.run('alias', function (alias) {
                                        return alias(payload.data.newId, payload.context);
                                    })];
                            case 'group':
                                return [2 /*return*/, this.wrapper.run('group', function (group) {
                                        return group(payload.data.groupId, payload.data.traits, payload.context);
                                    })];
                            case 'identify':
                                return [2 /*return*/, this.wrapper.run('identify', function (identify) {
                                        return identify(payload.data.user, payload.data.traits, payload.context);
                                    })];
                            case 'screen':
                                return [2 /*return*/, this.wrapper.run('screen', function (screen) {
                                        return screen(payload.data.name, payload.data.properties, payload.context);
                                    })];
                            case 'track':
                                return [2 /*return*/, this.wrapper.run('track', function (track) {
                                        return track(payload.data.event, payload.data.properties, payload.context);
                                    })];
                            default:
